...
function App() {
  ...
  const stats = useMemo(() => {
    ...
    const mean = selectedDice.reduce((sum, color) => sum + DICE_STATS[color].expectedValue, 0);
    ...
    for (const color of selectedDice) {
      ...
      for (const [currentDamageStr, currentProb] of Object.entries(dp)) {
        ...
        for (const [dieDamageStr, dieProb] of Object.entries(dieProbs)) {
          ...
          newDp[totalDamage] = (newDp[totalDamage] || 0) + (currentProb * dieProb);
        }
      }
      dp = newDp;
    }
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-gray-100 p-4 font-sans flex flex-col items-center">
      <header className="mb-6 text-center">
        ...
        <p className="text-gray-400 text-sm mt-1">Dice Probability Calculator</p>
      </header>

      <main className="w-full max-w-md space-y-6">
        {/* Dice Selection */}
        <div className="bg-gray-800 p-4 rounded-xl shadow-lg border border-gray-700">
          <h2 className="text-sm font-semibold text-gray-400 mb-3 uppercase tracking-wider">Add Dice</h2>
        ...
        <div className="bg-gray-800 p-4 rounded-xl shadow-lg border border-gray-700 min-h-[100px]">
           <div className="flex justify-between items-center mb-3">
            <h2 className="text-sm font-semibold text-gray-400 uppercase tracking-wider">Selected Pool ({selectedDice.length})</h2>
            {selectedDice.length > 0 && (
              <button onClick={resetDice} className="text-xs text-red-400 hover:text-red-300 underline">
                Reset
              </button>
            )}
           </div>
           ...
           {selectedDice.length === 0 ? (
             <div className="text-center text-gray-600 py-4 italic">No dice selected</div>
           ) : (
        ...
        <div className="bg-gray-800 p-4 rounded-xl shadow-lg border border-gray-700">
          <h2 className="text-sm font-semibold text-gray-400 mb-2 uppercase tracking-wider">Monster Health</h2>
          <input
            type="number"
            min="1"
            value={monsterHealth}
            onChange={(e) => setMonsterHealth(e.target.value === '' ? '' : parseInt(e.target.value))}
        ...
        <div className="bg-gray-800 p-6 rounded-xl shadow-lg border border-gray-700 space-y-4">
          <div className="flex justify-between items-end border-b border-gray-700 pb-4">
            ...
            <span className="text-3xl font-bold text-white">{stats.mean.toFixed(2)}</span>
          </div>
          
          <div className="flex justify-between items-end">
            <span className="text-gray-400">Kill Probability</span>
            <span className={`text-4xl font-bold ${stats.killProbability > 0.8 ? 'text-green-400' : stats.killProbability > 0.5 ? 'text-yellow-400' : 'text-red-400'}`}>
              {(stats.killProbability * 100).toFixed(1)}%
            </span>
          </div>
          {monsterHealth === '' && (
            <p className="text-xs text-gray-500 text-center mt-2">Enter Monster Health to see probability</p>
          )}
        </div>
      </main>
    </div>
